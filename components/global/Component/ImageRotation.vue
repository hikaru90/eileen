...
<template>
  <div class="mb-24">
    ...
    <div class="flex flex-col lg:flex-row lg:items-stretch">
      <div class="relative w-full lg:w-1/2 lg:mr-10 bg-cover">
        <div
      ...
      <div class="lg:w-1/2">
        <div
          v-for="(slide, index) in props.component.content.slides"
          :key="'slide' + index"
          class="slide flex items-start gap-4 transition-opacity delay-300 relative"
          :class="[index === state.currentSlide ? 'opacity-100' : 'opacity-60']"
        >
          <div class="w-6 h-6 mt-1px flex-shrink-0 rounded-full flex items-center justify-center">
          ...
          <div style="width: 1px" class="connector h-full absolute top-1 left-3 bg-gold mb-1"></div>
          <div>
            <a :href="slide.link" class="flex items-center gap-2 mb-2">
              <h3 class="font-bold lg:text-lg ">
                {{ slide.text }}
              </h3>
              <div class="w-4 h-4 bg-black rounded-full flex items-center justify-center text-gold">
