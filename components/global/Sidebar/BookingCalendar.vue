...
<template>
  <div class="p-4">
    <div v-for="(day, index) in componentContent.days">
      <h2 class="text-xs mb-2 opacity-40">{{ day.name }}</h2>
      <div class="flex gap-2 mb-2">
        <div>
          <h2 class="text-xs opacity-40">von</h2>
          <select
            @change="saveContent"
        ...
        <div>
          <h2 class="text-xs opacity-40">bis</h2>
          <select
            @change="saveContent"
            v-model="day.endTime"
            class="w-full rounded-sm pl-1 bg-transparent border border-darkOffwhite border-opacity-20"
