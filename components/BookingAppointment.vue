...
<template>
  <div class="select-none border border-darkGrey border-opacity-40 bg-offwhite rounded">
    <button
    ...
    <div v-if="state.isOpen" class="p-4 border-t border-grey border-opacity-20 flex flex-col">
      ...
      <div class="bg-white rounded p-2 shadow-lg mt-4 flex items-center justify-between">
          <button v-show="!state.deletionConfirmed" @click="state.deletionConfirmed = true" class="border-2 border-red/50 hover:border-red text-red/70 hover:text-red rounded px-4 py-1 flex items-center">
            <nuxt-icon name="icon-trash" class="text-2xl -ml-2" />
            Löschen
          </button>
          <button v-show="state.deletionConfirmed" @click="deleteBooking()" v-on-click-outside="cancelDeletion" class="border-2 border-red/50 hover:border-red text-red/70 hover:text-red rounded px-4 py-1 flex items-center">
            <nuxt-icon name="icon-check" class="text-2xl -ml-2" />
            Sicher?
          </button>
        <button @click="approveBooking" class="border-2 border-green/70 hover:border-green text-green/70 hover:text-green rounded px-4 py-1 flex items-center">
          <nuxt-icon name="icon-check" class="text-2xl -ml-2" />
          Bestätigen
        </button>
      </div>
