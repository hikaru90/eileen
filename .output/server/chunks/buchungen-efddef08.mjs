import _sfc_main$4 from './nuxt-icon-14b11dc2.mjs';
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(state).isOpen) {
        ...
        ssrRenderList(unref(state).formEntries, (entry, index) => {
          _push(`<div class="flex items-center justify-between border-b border-grey border-opacity-20 last:border-b-0 py-1 px-2"><div class="">${ssrInterpolate(entry.name)}</div><div class="font-bold">${ssrInterpolate(entry.value)}</div></div>`);
        });
        _push(`<!--]--></div>`);
        if (props.booking.deleted) {
          _push(`<div class="bg-white rounded p-2 shadow-lg mt-4 flex items-center justify-center"><button class="border-2 border-black/70 hover:border-black text-black/70 hover:text-black rounded px-4 py-1 flex items-center"> Reaktivieren </button></div>`);
...
//# sourceMappingURL=buchungen-efddef08.mjs.map
